...
def simple_model():
    model = nn.Sequential(
        nn.Conv1d(in_channels=1, out_channels=128, kernel_size=50, stride=4, bias=False),
        ...
        nn.Dropout(p=0.5),
        nn.Flatten(),
        nn.Linear(in_features=2688, out_features=5, bias=False)
    )
    return model
...
class SimpleModel:
    ...
    def train(self, minibatch_fn):
        ...
        for x, y, w, sl, re in minibatch_fn:
            ...
            y = torch.from_numpy(y).long().to(self.device)  # shape(batch_size * seq_length, )
